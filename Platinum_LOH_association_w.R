...
for (i in 1:length(idxnormal1)){
  ...
  if (file.exists(output))
  {
    ...
    LOHseg[count,1] <- sum(dat$dhEnd[!is.na(dat$lohCall) & dat$lohCall]-dat$dhStart[!is.na(dat$lohCall) & dat$lohCall])/10^6
    LOHseg[count,2] <- length(dat$dhEnd[!is.na(dat$lohCall) & dat$lohCall])
    LOHseg[count,3] <- sum(dat$tcnEnd[!is.na(dat$tcnStart) & is.finite(dat$tcnStart) & is.finite(dat$tcnEnd)] -dat$tcnStart[!is.na(dat$tcnStart) & is.finite(dat$tcnStart) & is.finite(dat$tcnEnd)])/10^6
    LOHseg[count,4] <- length(dat$tcnEnd[!is.na(dat$tcnStart) & is.finite(dat$tcnStart) & is.finite(dat$tcnEnd)])
    availablesamples=c(availablesamples,mysample)
    count=count+1
  }    
}
LOHseg=LOHseg[1:length(availablesamples),]
loh <- data.frame(cbind(availablesamples,LOHseg))
loh[,1] <- as.character(loh[,1])
loh <- loh[order(loh[,1]),]
names(loh) <- c("sampleID","loh_length","n_lohsegs","tcn_length","n_tcnsegs")

save(loh,file="/fh/fast/dai_j/CancerGenomics/Ovarian_Cancer/result/TCGA_lohX.RData")
...
summary(glm(I(resistance=="Sensitive")~loh_length,family=binomial,data=resist)) #0.000287
summary(glm(I(resistance=="Sensitive")~n_lohsegs,family=binomial,data=resist))   #0.0244
...
for (mysample in availablesamples)
{
  tmp=read.table(file=paste0("/fh/fast/dai_j/CancerGenomics/Ovarian_Cancer/result/tmp/",mysample,".allele.PSCBS.segments.txt"),header=T)
  names(tmp)[1] <- "ID"
...
save(segsall_cn,segsall_loh,resist,file="/fh/fast/dai_j/CancerGenomics/Ovarian_Cancer/result/tmp/tcga_pscbs_segment.RData")
...
save(TCGA_gene_loh,file="/fh/fast/dai_j/CancerGenomics/Ovarian_Cancer/result/TCGA_gene_lohX.RData")
...
save(outp,file="/fh/fast/dai_j/CancerGenomics/Ovarian_Cancer/result/TCGA_platinum_cnlohX_pvalues.RData")
